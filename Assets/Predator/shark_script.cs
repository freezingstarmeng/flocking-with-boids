...
public class shark_script : MonoBehaviour {
	...
	private bool checkBoundry() {
		...
		if (Vector3.Distance(simulation_centre, transform.position) > simulation_radius && (hunger < hunger_threshold)) {
			...
			GetComponent<Rigidbody>().AddForce((-speed*2) * Vector3.Normalize(transform.position - simulation_centre));			
			return true;
		}
		return false;
	}
	...
	private void chaseFish() {
		if (hunger >= hunger_threshold) {
			...
			GetComponent<Rigidbody>().AddForce(-speed * Vector3.Normalize(transform.position - fishPosition));
			transform.rotation = Quaternion.LookRotation(newDir);
