...
public class fish_script : MonoBehaviour {
	...
	private bool checkBoundry() {
		...
		if (Vector3.Distance(simulation_centre, transform.position) > simulation_radius) {
			...
			GetComponent<Rigidbody>().AddForce(-speed * Vector3.Normalize(transform.position - simulation_centre));
			return true;
		}
		return false;
	}
	...
	private void runFromSharks() {
		if (sharkDistance < comfort_distance) {
			...
			transform.rotation = Quaternion.LookRotation(newDir);
			GetComponent<Rigidbody>().AddForce(speed * Vector3.Normalize(transform.position - sharkPosition));
		}
	}
